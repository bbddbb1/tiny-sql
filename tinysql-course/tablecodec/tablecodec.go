...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	...
	if len(key) != RecordRowKeyLen {
		return 0, 0, errInvalidRecordKey.GenWithStack("key len err")
	}
	tp := key[:tablePrefixLength]
	key = key[tablePrefixLength:]
	//fmt.Printf("de recordPrefix %v recordPrefix %v", tp, tablePrefix)
	for i, v := range tp {
		if v != tablePrefix[i] {
			return 0, 0, errInvalidRecordKey.GenWithStack("tablePrefix err")
		}
	}
	remain, tableID, err := codec.DecodeInt(key)
	if err != nil {
		return 0, 0, err
	}
	rp := remain[:recordPrefixSepLength]
	remain = remain[recordPrefixSepLength:]
	//fmt.Printf("de recordPrefix %v recordPrefix %v", rp, recordPrefixSep)
	for i, v := range rp {
		if v != recordPrefixSep[i] {
			return 0, 0, errInvalidRecordKey.GenWithStack("recordPrefix err")
		}
	}
	_, handle, err = codec.DecodeInt(remain)
	if err != nil {
		return 0, 0, err
	}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	...
	if len(key) <= prefixLen+idLen {
		return 0, 0, nil, errInvalidRecordKey.GenWithStack("key err")
	}
	tp := key[:tablePrefixLength]
	key = key[tablePrefixLength:]
	//fmt.Printf("de recordPrefix %v recordPrefix %v", tp, tablePrefix)
	for i, v := range tp {
		if v != tablePrefix[i] {
			return 0, 0, nil, errInvalidRecordKey.GenWithStack("tablePrefix err")
		}
	}
	remain, tableID, err := codec.DecodeInt(key)
	if err != nil {
	...
	indexPrefixSeplen := len(indexPrefixSep)

	ip := remain[:indexPrefixSeplen]
	remain = remain[indexPrefixSeplen:]
	//fmt.Printf("de recordPrefix %v recordPrefix %v", rp, recordPrefixSep)
	for i, v := range ip {
		if v != indexPrefixSep[i] {
			return 0, 0, nil, errInvalidRecordKey.GenWithStack("indexPrefixSep err")
		}
	}
	indexValues, indexID, err = codec.DecodeInt(remain)
	if err != nil {
		return 0, 0, nil, err
	}
